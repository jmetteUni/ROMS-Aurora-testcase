...
griddir = '/home/jonathan/Dokumente/model/Gridbuilder';
gridname = 'Grid_Aurora_M55L55.nc';
grid = fullfile(griddir,gridname);
% Base date for ROMS forcing files
mybasedate = datenum(2021,30,05,0,0,0);
dates = datenum(2021,30,05,00,0,0);
datee = datenum(2021,30,06,00,0,0);
frc_date = dates:3:datee;
eruption_date = dates;
heat_vent = 90000000; % Aurora, arbitrary   %from poster:20MW %from Wegener2023: 130MW
...
outdir = '/home/jonathan/Dokumente/model/point_source';
if ~exist(outdir,'dir')
    mkdir(outdir)
end
...
F(1).output = sprintf('%s_bhflux_%dMW_%s_large_heat3_%s.nc',gridname(1:end-3),heat_vent,coarsefine,datestr(eruption_date,'yyyymmddTHH'));
F(1).Vname = 'bhflux';
F(1).Tname = 'bhf_time';
%F(2).output = sprintf('%s_bwflux_Kellog_120day_1_vent.nc',gridname(1:end-3));
%F(2).Vname = 'bwflux';
%F(2).Tname = 'bwf_time';
% F(2).output = sprintf('%s_bpflux_large_heat3_%s.nc',gridname(1:end-3),datestr(eruption_date,'yyyymmddTHH'));
    ...
    for ivent = 1:length(lon_vent)
        ...
        for ix = 1:size(ix_vent,1)
            for iy = 1:size(iy_vent,1)
                ...
                % bp_max = 10*heat_vent(ivent)/max(heat_vent);
                % bpflux1 = bp_max*ones(1,length(frc_time));
                % bpflux1(frc_time<0) = 0;
                % bpflux(is1,js1,:) = bpflux1;
                % bw_max = 0*heat_vent(ivent)/max(heat_vent);
                % bwflux1 = bw_max*ones(1,length(frc_time));
