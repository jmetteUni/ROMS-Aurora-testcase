...
GRD_dir  = 'Dokumente/model/Gridbuilder/';
GRD_file = 'Grid_Aurora_M55L55.nc';
GRDname = fullfile(GRD_dir,GRD_file);
NEMO_dir = 'Dokumente/model/boundary_conditions';
NEMO_name = 'cmems_mod_glo_phy_my_0.083deg_P1D-m_1711116131061.nc';
NEMOfile = fullfile(NEMO_dir,NEMO_name);


% set output file directory and name
INI_dir = '/media/jonathan/Extreme SSD/';

% set reference date
time_nemo = ncread(NEMOfile,'time',1,1);    %hours since 1950-01-01
Pdate = double(time_nemo)/24+datenum(1950,1,1);
date_ref = Pdate;

% set output file name
INI_file = sprintf('%s_ini_nemo_%s_64layer.nc',GRD_file(1:end-3),datestr(Pdate,'yyyymmddTHH'));
INIname = fullfile(INI_dir,INI_file);
...
if (CREATE)
    ...
    avalue='seconds since 2021-05-30 00:00:00';
