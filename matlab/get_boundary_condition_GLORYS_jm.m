...
GRD_dir  = 'Dokumente/model/Gridbuilder/';      %check directory
...
date_ref = datenum(2021,05,30,00,00,00);
date_start = date_ref;
date_end = datenum(2021,06,30,00,00,00);
date_nemo=linspace(date_start,date_end,32);

% set output file directory and name
BRY_dir = '/media/jonathan/Extreme SSD/';
BRY_file = sprintf('%s_bry_nemo_%s_%s.nc',GRD_file(1:end-3),datestr(date_start,'yyyymmddTHH'),datestr(date_end,'yyyymmddTHH'));
BRYname = fullfile(BRY_dir,BRY_file);
...
Sinp.N = 15;                    % number of vertical levels at RHO-points
Sinp.Vtransform = 2;
Sinp.Vstretching = 4;
Sinp.theta_s=0;
Sinp.theta_b=1;
